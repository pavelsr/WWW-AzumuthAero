        ...
        for my $flight_dom ( $target_dom->find('div.sf-flight-block')->each ) {
            ...
            if ($stops_css) {
                if ( $stops_css->text =~ /пересадк/ ) {
                    ...
                    warn "Dur: ".fix_html_string $flight_dom->at('div.ts-flight__duration div.ts-flight__dur')->text;
...
sub get_schedule_dates {
    ...
    if ( ref( $res->{available_to} ) eq 'ARRAY' ) {
        ...
        for my $interval ( @{ $res->{available_to} } ) {
            ...
            # $interval->{max}
            # $interval->{days}
            push @dates, get_dates_from_dows(%$interval);    # to-do : check min_date and max_date
        }
...
sub get_fares_schedule {
    ...
    say $params{from}.'->'.$params{to}.' : '.$params{min}.'->'.$params{max} if $params{progress_bar};
    for my $i ( 0 .. $#available_dates ) {

        my $date_str = $available_dates[$i];
        
        say ''. ( $i + 1 ) . '/' . scalar @available_dates if $params{progress_bar};
    ...
    if ( $params{print_table} ) {
        say $_->as_string( order => [qw/flight_date/], separator => "\t" ) for (@fares);
    }
